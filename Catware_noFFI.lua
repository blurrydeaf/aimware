	...
	local rb_tab			= gui.Tab(rb_reference, "catware_script", "CATWARE.lua")
		local rb_box			= gui.Groupbox(rb_tab, "Welcome Semirager! ", 10, 5, 300, 1)
			...
			local rb_unsafe			= gui.Checkbox(rb_box, "catware_unsafe", "Enable Unsafe", false)
				rb_unsafe:SetDescription("*WARNING* Lower your trust factor")
...
local rb_feature		= gui.Groupbox(rb_tab, "Feature", 320, 290, 300, 1)
	...
	--local rb_clantag		= gui.Checkbox(rb_feature, "catware_clantag", "Enable Clantag", false);
...
local rb_antiaim		= gui.Groupbox(rb_tab, "AntiAim", 10, 160, 300, 1)
	...
	local rb_syaw			= gui.Slider(rb_antiaim, "catware_syaw", "Yaw", 0, 0, 50, 1)
	local rb_sroll			= gui.Slider(rb_antiaim, "catware_sroll", "Roll", 0, 0, 45, 1)
	local rb_sfreestand		= gui.Slider(rb_antiaim, "catware_sfreestand", "Freestand", 0, 0, 90, 1)
...
--########## UNLOCK INVENTORY THE SERVERS VALVE ########## coded by public user
...
local function pitch_func(cmd)
	...
	if weapon_id > 42 and weapon_id < 49 then return end -- attempt to compare number with nil
...
--##########  FUNCTION JUMPSTABLE ##########
local function jumpstable_func()

	local localplayer = entities.GetLocalPlayer()
	
	if localplayer and localplayer:IsAlive() then
	
		local vel = math.sqrt(localplayer:GetPropFloat("localdata", "m_vecVelocity[0]") ^ 2 + localplayer:GetPropFloat("localdata", "m_vecVelocity[1]") ^ 2) --coded by public  user
		
		if vel < -20 or vel > 20 then
...
--[[
--##########  FUNCTION CLANTAG ##########
local ffi_clan_tag					= ffi.cast('int(__fastcall*)(const char*, const char*)', mem.FindPattern("engine.dll", "53 56 57 8B DA 8B F9 FF 15"))
local last = nil						
...
local SetTag = function(v)		-- coded by cheese
  if v ~= last then
    ffi_clan_tag(v, "")
    last = v
  end
...
local function makesteps()
	  ...
	  for i = 1, #current_tag do
		table.insert(tagsteps, current_tag:sub(1, i))
	  end
...
local function set_clan_tag()
	...
	if not rb_clantag:GetValue() then return end
	...
	if rb_clantag:GetValue() then
		current_tag = clan_tags.default
	end
	
	if lasttag ~= current_tag then
      makesteps()
      lasttag = current_tag
    end
	
   SetTag(tagsteps[round(globals.TickCount()/((6.5-1.5)*3.0))%(#tagsteps-1)+1])
